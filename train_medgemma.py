"""SFT training script for MedGemma-4B-IT with confidence calibration (v8 - KL Anchor Optimized).

This script fine-tunes MedGemma-4B-IT to predict a single-digit confidence
score (0-9) for multiple-choice questions using Brier loss or EGC loss on
the confidence digit.

v7 Change: Added KL-divergence anchoring to base model on answer tokens.
This preserves the base model's answering behavior while learning calibration.

v8 Change: Optimized KL anchor loss computation using vectorized operations
with masking instead of per-position Python loop. Exploits fixed end positions
from left-padding while handling variable start positions via mask.

The training pipeline supports any dataset with grouped perturbation structure:

* Dataset: Any dataset with `group_id`, `full_text`, `image_path`, `correct` columns.
  Supported datasets: OmniMedVQA, PMC-VQA.
* Model: MedGemma-4B-IT loaded via `AutoModelForImageTextToText`.
* Loss: Brier loss or EGC (Expected Gradient Calibration) loss.
* Confidence: confidence digit is the token immediately after the confidence prefix.
"""
...
# Model-specific constants for answer region detection (KL anchor)
MODEL_CONSTANTS = {
	"medgemma": {
		# "Rationale:" tokenizes to [224693, 236787] in MedGemma
		"rationale_token_ids": [224693, 236787],
		# Exclude last 4 tokens ("Confidence: X") for MedGemma
		"answer_end_offset": 4,
		# Digits 0-10 (11 tokens), scores = i/10
		"num_digits": 11,
		"score_divisor": 10.0,
		"model_class": "AutoModelForImageTextToText",
	},
	"qwen3": {
		# "Rationale:" tokenizes to [49, 37035, 25] in Qwen3
		"rationale_token_ids": [49, 37035, 25],
		# Exclude last 5 tokens ("Confidence: X") for Qwen3
		"answer_end_offset": 5,
		# Digits 0-9 (10 tokens), scores = i/9 (T_9 normalization)
		"num_digits": 10,
		"score_divisor": 9.0,
		"model_class": "AutoModelForImageTextToText",
	},
...
# These globals are set in main() based on --model_type
RATIONALE_TOKEN_IDS = MODEL_CONSTANTS["medgemma"]["rationale_token_ids"]
ANSWER_END_OFFSET = MODEL_CONSTANTS["medgemma"]["answer_end_offset"]


@dataclass
class TrainConfig:
	"""Configuration for MedGemma confidence SFT.

	Supports multiple datasets (OmniMedVQA, PMC-VQA) with grouped perturbation structure.
	"""

	model_name: str = "google/medgemma-4b-it"
	output_dir: str = "./outputs_medgemma_omnimedvqa"
	model_type: str = "medgemma"  # "medgemma" or "qwen3"
	...
	wandb_project: str = "medgemma_uncertainty_sft"
...
def setup_tokenizer_and_model(train_config: TrainConfig):
	"""Load processor and MedGemma-4B-IT model for training."""
	...
	if train_config.freeze_vision_encoder:
		...
		# Freeze all parameters that contain 'vision_tower' or 'vision_model' in their name
...
def build_digit_token_indices(tokenizer, device, num_digits: int = 11) -> torch.Tensor:
	"""Build token ID list for digits 0..N-1 for the given tokenizer.

	Args:
		tokenizer: The tokenizer to use.
		device: Target device.
		num_digits: Number of digits (11 for 0-10 MedGemma, 10 for 0-9 Qwen3).
	"""

	# [TRACE] Step 1: Select the tokens corresponding to the digits.
	digits = [str(i) for i in range(num_digits)]
...
def load_base_model_for_kl(train_config: TrainConfig, device: torch.device):
	"""Load a frozen copy of the base model for KL-anchor regularization.

	The base model is loaded with the same dtype as the trainable model,
	kept in eval mode with no gradients.
	"""
	print("Loading frozen base model for KL-anchor regularization...")
	...
	for param in base_model.parameters():
		param.requires_grad = False
	print(f"Base model loaded and frozen on {device}")
	return base_model
...
def find_answer_token_positions(
	...
	IMPORTANT: This function assumes LEFT padding (MedGemma default).
	With left padding, ALL sequences end at position (max_len - 1),
	so "Confidence: X" is always at positions (max_len - 4) to (max_len - 1).

	Searches for the "Rationale:" token sequence (RATIONALE_TOKEN_IDS) and returns
	...
	for b in range(batch_size):
		# Search for the RATIONALE_TOKEN_IDS pattern
		found_pos = -1
...
def train_loop(train_config: TrainConfig) -> None:
	...
	mcfg = MODEL_CONSTANTS[train_config.model_type]
	digit_indices = build_digit_token_indices(processor.tokenizer, device, num_digits=mcfg["num_digits"])

	# Load frozen base model for KL-anchor if enabled
	base_model = None
	if train_config.use_kl_anchor:
		base_model = load_base_model_for_kl(train_config, device)
	...
	# Map digit tokens to [0, 1] range:
	#   MedGemma: 0-10 → i/10 (11 values)
	#   Qwen3:    0-9  → i/9  (10 values, T_9 normalization)
	# [TRACE] Step 2: Define numerical values (scores) to match the tokens.
	scores = torch.arange(mcfg["num_digits"], device=device).float() / mcfg["score_divisor"]
	...
	for epoch in range(train_config.num_epochs):
		if train_config.sanity_check:
			...
			with torch.no_grad():
				...
				# Note: MedGemma/PaliGemma usually expects 'pixel_values' and 'input_ids'
		...
		for batch in train_pbar:
			...
			loss_kl = None
			if train_config.use_kl_anchor and base_model is not None:
				with torch.no_grad():
					with torch.autocast(device_type="cuda", dtype=autocast_dtype):
			...
			num_conf = torch.index_select(num_token, 1, digit_indices)  # [B_eff, num_digits]
			...
			scores_expanded = scores.expand(num_conf.size(0), -1)  # [B_eff, num_digits]
			...
			if train_config.loss_type == "egc":
				...
				if train_config.brier_anchor_lambda < 1.0:
					...
					anchor_sq_error = (scores_expanded - 0.5) ** 2  # [B, num_digits]
			...
			pred_conf = torch.argmax(num_conf, dim=1).float() / mcfg["score_divisor"]
		...
		if train_preds_list:
			...
			if accelerator.is_main_process:
				...
				train_ece, bin_counts, bin_accuracies = compute_ece(train_labels_np, train_preds_np, source_scale=int(mcfg["score_divisor"]))
		...
		if eval_dataloader is not None:
			...
			if eval_preds_list:
				...
				if accelerator.is_main_process:
					...
					eval_ece, _, _ = compute_ece(eval_labels_np, eval_preds_np, source_scale=int(mcfg["score_divisor"]))
...
def parse_args() -> TrainConfig:
	...
	parser = argparse.ArgumentParser(
		description="SFT MedGemma-4B-IT with confidence calibration (Brier or EGC loss)",
	)

	parser.add_argument("--model_name", type=str, default="google/medgemma-4b-it")
	parser.add_argument("--output_dir", type=str, default="./outputs_medgemma_omnimedvqa")
	...
	parser.add_argument("--wandb_project", type=str, default="medgemma_uncertainty_sft")
	...
	parser.add_argument("--model_type", type=str, default="medgemma", choices=["medgemma", "qwen3"],
		help="Model type for token-specific constants (rationale tokens, digit range, etc.)")
...
def main() -> None:
	...
	# Set global constants based on model type
	global RATIONALE_TOKEN_IDS, ANSWER_END_OFFSET
	mcfg = MODEL_CONSTANTS[train_config.model_type]
	RATIONALE_TOKEN_IDS = mcfg["rationale_token_ids"]
	ANSWER_END_OFFSET = mcfg["answer_end_offset"]
	print(f"Model type: {train_config.model_type}")
	print(f"  RATIONALE_TOKEN_IDS = {RATIONALE_TOKEN_IDS}")
	print(f"  ANSWER_END_OFFSET = {ANSWER_END_OFFSET}")
	print(f"  Digit range: 0-{mcfg['num_digits']-1} ({mcfg['num_digits']} tokens)")
	print(f"  Score normalization: i/{mcfg['score_divisor']}")
